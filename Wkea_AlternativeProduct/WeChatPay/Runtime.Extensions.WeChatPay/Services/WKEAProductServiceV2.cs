namespace Contoso.Commerce.Runtime.WeChatPay
{
    ...
    public class WKEAProductServiceV2 : IRequestHandlerAsync
    {
        ...
        public async Task<Response> Execute(Request request)
        {
            ...
            {
                return await this.AddProductExtensionData((WKEAProductRequest)request);
            }
            else
            {
        ...
        private async Task<Response> AddProductExtensionData(WKEAProductRequest request)
        {
            ...
            if (request.ListSimpleProduct != null)
            {
                ...
                using (DatabaseContext databaseContext = new DatabaseContext(request.RequestContext))
                {
                    ...
                    {
                        SqlPagedQuery query = new SqlPagedQuery(QueryResultSettings.FirstRecord)
                        {
                            DatabaseSchema = "ext",
                            From = "WKEA_INVENTITEMPURCHSETUP"
                        };

                        query.Parameters["@ItemID"] = iSimpleProduct.ItemId;

                       // databaseContext.ReadEntityAsync<>
